    ...
    for epoch in range(args.startEpoch, args.startEpoch + args.epochs):
        ...
        if epoch >= args.mixupEpoch:   
            if (epoch == args.mixupEpoch) or ((epoch - args.mixupEpoch) % args.recomputePrototypeInterval == 0):
                ...
                model.computePrototype_featuremap(model, train_loader, args)
                logger.info('Done!')
...
def Train(train_loader, model, criterion, optimizer, writer, epoch, args):
    ...
    crossFeatureDistance = {'pos2pos': AverageMeter(), 'pos2neg': AverageMeter(), 'neg2neg': AverageMeter()}
    ...
    for batchIndex, (sampleIndex, input, target, groundTruth) in enumerate(train_loader):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if epoch < args.proposedMethodEpoch:
                ...
                mixedProb2.update(torch.sum((mixedTarget2 > 0).float()).item(), args.batchSize * args.classNum)
        ...
        loss3_ = 0 * loss1_ if epoch < args.proposedMethodEpoch else args.instanceLossWeight * criterion['BCELoss'](mixedOutput1, mixedTarget1)
        loss4_ = 0 * loss1_ if epoch < args.proposedMethodEpoch else args.prototypeLossWeight * criterion['BCELoss'](mixedOutput2, mixedTarget2)
        ...
        if batchIndex % args.printFreq == 0:
            logger.info('[Train] [Epoch {0}]: [{1:04d}/{2}] Batch Time {batch_time.avg:.3f} Data Time {data_time.avg:.3f} Learn Rate {lr:.6f} alpha {alpha:.4f} beta {beta:.4f}\n'
                        '\t\t\t\t\tProposed Method: Mixed Prob1 {mixedProb1.avg:.4f} Mixed Prob2 {mixedProb2.avg:.4f} Positive Prob {positiveProb.avg:.4f}\n'
                        '\t\t\t\t\tBCE Loss {loss1.val:.4f} ({loss1.avg:.4f}) Contrastive Loss {loss2.val:.4f} ({loss2.avg:.4f}) Instance Loss {loss3.val:.4f} ({loss3.avg:.4f}) Prototype Loss {loss4.val:.4f} ({loss4.avg:.4f})'.format(
                        ...
                        mixedProb1=mixedProb1, mixedProb2=mixedProb2, positiveProb=positiveProb,
    ...
    writer.add_scalar('Pos2Pos', crossFeatureDistance['pos2pos'].avg, epoch)
    writer.add_scalar('Pos2Neg', crossFeatureDistance['pos2neg'].avg, epoch)
    writer.add_scalar('Neg2Neg', crossFeatureDistance['neg2neg'].avg, epoch)
